...
const SOLO_QUESTION_MS = 18000;
const DUEL_QUESTION_MS = 30000;
...
function resetRoundState() {
  ...
  state.onlineGameStarted = false;
  state.onlineTransitionPending = false;
  state.onlineResultsShown = false;
  state.answerCache = {};
...
function renderTextInputQuestion(question, existingAnswer = null) {
  ...
  wrap.innerHTML = `
    ...
    <input id="textAnswerInput" type="text" placeholder="Digite sua resposta" />
...
function showPendingOnlineAnswer() {
  elements.feedbackBanner.textContent = "Resposta enviada. Aguardando o outro jogador.";
  elements.feedbackBanner.className = "feedback-banner";
}
...
function animateSoloTimer() {
  ...
  const tick = () => {
    ...
    const ratio = Math.max(0, 1 - elapsed / SOLO_QUESTION_MS);
    elements.timerBar.style.transform = `scaleX(${ratio})`;
    if (elapsed >= SOLO_QUESTION_MS) submitCurrentAnswer(null, true);
  };
  tick();
...
function submitCurrentAnswer(rawValue, timedOut = false) {
  const question = getCurrentQuestion();
  if (state.mode === "solo") answerSolo(question, rawValue, timedOut);
  else if (state.mode === "online") submitOnlineAnswer(question, rawValue, timedOut);
}
...
function answerSolo(question, rawValue, timedOut = false) {
  ...
  const elapsedMs = timedOut ? SOLO_QUESTION_MS : Math.min(SOLO_QUESTION_MS, performance.now() - state.questionStartedAt);
...
function watchRoom(code) {
  ...
  state.roomUnsubscribe = state.firebase.onValue(roomRef(code), (snapshot) => {
    ...
    if (state.room.status === "playing" && !state.onlineGameStarted) startOnlineGame();
    if (state.room.status === "finished" && state.onlineGameStarted && !state.onlineResultsShown) finishOnlineGame();
  });
}
...
function renderLobby() {
  ...
  if (state.isHost) {
    ...
    elements.lobbyHint.textContent = players.length === 2
      ? "Os dois jogadores estão prontos. Cada pergunta terá 30 segundos."
      : "Abra o jogo em outro navegador, celular ou aba anônima para testar.";
  } else {
    elements.startDuelButton.classList.add("hidden");
    elements.lobbyHint.textContent = "Aguardando o criador da sala começar. Cada pergunta terá 30 segundos.";
  }
}
...
async function startOnlineGame() {
  ...
  if (getServerNow() < room.startAt) await runCountdown("30 segundos ou até os dois responderem.", room.startAt);
  showScreen("screen-game");
  runOnlineLoop();
}

function currentRoundPlayerIds(room = state.room) {
  return activePlayerEntries(room).map(([id]) => id).slice(0, 2);
}

function didBothPlayersAnswer(room, index) {
  const playerIds = currentRoundPlayerIds(room);
  return playerIds.length === 2 && playerIds.every((id) => room?.answers?.[id]?.[index] !== undefined);
}
...
function runOnlineLoop() {
  ...
  const loop = () => {
    ...
    const index = Math.max(0, Number(room.currentQuestionIndex || 0));
    if (index >= total) {
      advanceOnlineRound(Math.max(0, total - 1));
      state.onlineLoop = requestAnimationFrame(loop);
      return;
    }

    const phase = room.phase === "feedback" ? "feedback" : "question";
    const questionStartedAt = Number(room.questionStartedAt || room.startAt || now);
    const feedbackStartedAt = Number(room.feedbackStartedAt || now);
    ...
    if (state.renderedOnlineIndex !== index) {
      ...
      state.currentInputDraft = null;
      renderQuestion(state.questions[index], index, total, phase === "feedback" ? existingAnswer : null);
      if (phase === "question" && existingAnswer) {
        lockInteractiveArea();
        showPendingOnlineAnswer();
      }
    }

    if (phase === "question") {
      const elapsed = Math.max(0, now - questionStartedAt);
      const ratio = Math.max(0, 1 - elapsed / DUEL_QUESTION_MS);
      elements.timerBar.style.transform = `scaleX(${ratio})`;
      state.onlinePhaseKey = `question-${index}`;

      if (didBothPlayersAnswer(room, index) || elapsed >= DUEL_QUESTION_MS) {
        transitionToOnlineFeedback(index);
      }
    } else {
      elements.timerBar.style.transform = "scaleX(0)";
      const phaseKey = `feedback-${index}`;
      if (state.onlinePhaseKey !== phaseKey) {
        state.onlinePhaseKey = phaseKey;
        showOnlineFeedback(index);
      }
      if (now - feedbackStartedAt >= FEEDBACK_MS) advanceOnlineRound(index);
    }
...
async function transitionToOnlineFeedback(index) {
  if (state.onlineTransitionPending || !state.roomCode || !state.firebase) return;
  state.onlineTransitionPending = true;
  const now = getServerNow();

  try {
    await state.firebase.runTransaction(roomRef(state.roomCode), (room) => {
      if (!room || room.status !== "playing") return;
      if (Number(room.currentQuestionIndex || 0) !== index) return;
      if (room.phase === "feedback") return;
      ...
      const playerIds = Object.entries(room.players || {})
        ...
        .slice(0, 2)
        .map(([id]) => id);
      const bothAnswered = playerIds.length === 2 && playerIds.every((id) => room.answers?.[id]?.[index] !== undefined);
      const startedAt = Number(room.questionStartedAt || room.startAt || now);
      const expired = now - startedAt >= DUEL_QUESTION_MS;
      if (!bothAnswered && !expired) return;

      room.phase = "feedback";
      room.feedbackStartedAt = now;
      return room;
    });
  } catch (error) {
    console.warn("Não foi possível abrir o resultado da pergunta.", error);
  } finally {
    state.onlineTransitionPending = false;
  }
}

async function advanceOnlineRound(index) {
  if (state.onlineTransitionPending || !state.roomCode || !state.firebase) return;
  state.onlineTransitionPending = true;
  const now = getServerNow();
  ...
  try {
    await state.firebase.runTransaction(roomRef(state.roomCode), (room) => {
      ...
      const total = Number(room.questionCount || room.questionIds?.length || 0);
      if (nextIndex >= total) {
...
async function submitOnlineAnswer(question, rawValue, timedOut = false) {
  if (state.currentLocked || timedOut) return;
  const room = state.room;
  const index = state.currentIndex;
  if (!room || room.status !== "playing" || room.phase === "feedback") return;
  if (Number(room.currentQuestionIndex || 0) !== index) return;

  const roundStartAt = Number(room.questionStartedAt || room.startAt || getServerNow());
  const elapsedMs = Math.max(0, Math.min(DUEL_QUESTION_MS, getServerNow() - roundStartAt));
  if (elapsedMs >= DUEL_QUESTION_MS) return;
...
async function finishOnlineGame() {
  if (state.onlineResultsShown) return;
  state.onlineResultsShown = true;
  stopTimers();
...
function playerResult(instanceId, player) {
  ...
  const time = entries.reduce((sum, answer) => sum + (answer?.elapsedMs || DUEL_QUESTION_MS), 0)
    + (state.questions.length - entries.length) * DUEL_QUESTION_MS;
